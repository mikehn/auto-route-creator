...
function initServerPaths(route, app) {
  ...
  Object.keys(mockData).forEach(function (url) {
    Object.keys(mockData[url]).forEach(function (proto) {
      ...
      app[protocolStr](url, function (req, res, next) {
...
function startMock(routes, port) {
  ...
  app.listen(port, function () {
    return console.log("Mock server on port ".concat(port, "!"));
  });
}
...
function updateArrayData(arr, params) {
  ...
  var data = new Array(DEFAULT_DATA_SET_SIZE);
...
var autoMock = function autoMock(routes, port) {
  return startMock(routes, port || PORT, app);
};
