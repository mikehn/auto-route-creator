...
var DELETE = METHOD.DELETE,
    ...
    PUT = METHOD.PUT,
    PATCH = METHOD.PATCH;
var AppMethod = (_AppMethod = {}, _defineProperty(_AppMethod, DELETE, "delete"), _defineProperty(_AppMethod, GET, "get"), _defineProperty(_AppMethod, POST, "post"), _defineProperty(_AppMethod, PUT, "put"), _defineProperty(_AppMethod, PATCH, "patch"), _AppMethod);
...
function deepClone(val) {
  ...
  var currentKey = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
...
function initPath(proto, req, res) {
  ...
  if (!mockData[url] || !mockData[url][proto] || mockData[url][proto].data == DEFAULT_RES_MARKER) {
    if (defaultRes) {
      ...
      if (proto == METHOD.PUT) {
        mockData[url][METHOD.GET] = _objectSpread({}, mockData[url][METHOD.GET], {
          data: req.body
        });
        res.send(req.body);
      }
  ...
  var jsonRes = addDynamicValues(deepClone(mockObj.data), req, proto, resOverride, mockObj.filter);
...
function startMock(routes) {
  ...
  var httpsCredentials = options.https; // {key: privateKey, cert: certificate};

  dataSize = options.defaultListSize || DEFAULT_DATA_SET_SIZE;
  defaultRes = options.defaultRes;
  if (options.interceptor) app.use(function (req, res, next) {
    return options.interceptor(req, res, next, mockData);
  });
  ...
  initServerPaths(app);
  var server = httpsCredentials ? https.createServer(httpsCredentials, app) : http.createServer(app);
...
function getAllValuesBykey(data, key) {
  ...
  if (Array.isArray(data)) {
    ...
    for (var i = 0; i < allKeys.length; ++i) {
      curRes = getAllValuesBykey(data[allKeys[i]], key).dkey; // if (curRes.length < res) {
      // 	res = curRes;
      // }
    ...
    pathList.forEach(function (path) {
      ...
      dynamicKeys.forEach(function (key) {
        ...
        var _getAllValuesBykey = getAllValuesBykey(data, objKey),
            ...
            dkObj = _getAllValuesBykey.dkObj;
