...
var PROTOCOL = Symbol("PROTOCOL"); //Key represents : if current path segment is dynamiclly supplied (i.e. generated id)
...
function initRoutes(data) {
  ...
  if (name) {
    ...
    curRouteGetter = function curRouteGetter() {
      return new Route(name, protocol, query, prefix, isDynamic, data[DYNAMIC]);
    };
