...
function getParamNames(func) {
  ...
  var res = functionText
    .slice(functionText.indexOf('(') + 1, functionText.indexOf(')'))
    .match(ARG_NAMES);
  if (res === null) res = [];
  return res;
}
...
function getQueryString(queryFunc, queryParams) {
  ...
  if (Array.isArray(queryParams)) qObject = queryFunc(EMPTY, ...queryParams);
...
function pathOptions(pathArgs: any, queryParams?: any, bodyParams?: any) {
...
class Route {
  ...
  constructor(
    public name,
    public protocol = METHOD.GET,
    public query,
    public fatherRoute = null,
    public isDynamic = false,
    public dynamicKey = null,
    public pathParts = [],
    public bodyFunc = null
  ) {
    this.name = name;
  ...
  path(options: PathParams = {}) {
    ...
    if (pathArgLength != this.dynamicCount && !isMock) {
      //ERROR
      errorLog(
        'WARNING : received wrong amount of path arguments for ',
        this.name,
        ' [got:',
        pathArgLength,
        ' expected:',
    ...
    if (this.fatherRoute) {
      ...
      fPath = this.fatherRoute.path(
        Object.assign({ isMock }, pathOptions(fatherArgs))
      );
    }
    let name = this.name;
    if (this.isDynamic) {
      if (isMock) {
        ...
        if (!this.dynamicKey || !IS_STR(this.dynamicKey)) {
          ...
          if (IS_UNDEF(pathArgs[this.dynamicKey])) {
            errorLog(
              `Need to supply in options value for key [${this.dynamicKey}] for path part ${name}`
            );
          } else {
  ...
  setBody(data) {
    if (this.protocol === METHOD.GET) {
      warnLog(
        'body of GET request is mostly ignored, are you sure you meant to set body ?'
      );
    }
    this.bodyData = data;
  }
...
function initRoutes(
  ...
  if (name) {
    ...
    let defaultQuery = (...qParam) => {
      if (qParam.length > 0) {
        errorLog(
          'No query params were defined, yet received following params ',
          JSON.stringify(qParam, null, 2)
        );
      }
      return null;
    };
    ...
    if (typeof query !== 'function') {
      ...
      } else innerQuery = (queryParams) => queryParams;
    } else {
    ...
    //////////////////
    if (IS_STR(data[DYNAMIC])) pathParts.push(data[DYNAMIC]);
    else pathParts.push(data[NAME]);
    curRouteGetter = () =>
      new Route(
        name,
        protocol,
...
function getRoute(treePath, options: PathParams = {}) {
